    ...
    <BaseModal :show="show" @close="closeModal">
        <template #title>
            <div class="flex items-center">
                ...
                Reset Password for {{ guardianName }}
            </div>
        </template>
                ...
                <div class="flex items-start">
                    ...
                    <div class="text-sm text-blue-800">
                        ...
                        <ul class="mt-1 space-y-1">
                            <li>• The parent/guardian will be forced to change their password on next login</li>
...
<script>
export default {
    name: "GuardianPasswordResetModal",
}
</script>
...
const resetPassword = async () => {
    ...
    try {
        ...
        const response = await fetch(`/api/v1/app/guardians/${props.guardianUuid}/user`, {
            method: 'PATCH',
            headers: {
        ...
        if (response.ok) {
            emit('success', 'Password reset successfully. Parent/Guardian will be forced to change password on next login.')
            closeModal()
        } else {
