<!-- src/views/course/ReportCard.vue -->
<template>
    <div class="bg-white rounded-lg border px-4 py-5 mt-2">
        <p class="text-sm text-left mx-2 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <BaseButton v-if="false" design="primary" size="sm"
                    :disabled="items.length === 0" @click="items.length && printItem(items[0])" title="Print latest">
...
const items = computed(() => {
    ...
    // Now check publish status for each group and populate comments from student histories
    Object.values(grouped).forEach(group => {
        ...
        if (history) {
            ...
            group.isPublished = history.isPublished ?? history.is_published ?? false
        ...
        // For high school courses, credits_earned is already set correctly from the API
        // The backend provides credits_earned from either current_school_courses.credit_earned
        // or course_enrollments.credits_earned, so no recalculation is needed
...
function getMarkAsComplete(course, blk) {
    // Try to find mark_as_complete from student's currentSchoolHistories
    const studentHistories = props.student?.currentSchoolHistories || props.student?.current_school_histories || []
    
    // Find history matching school year and grade level
    const matchingHistory = studentHistories.find(h => {
        const syUuid = h.schoolYear?.uuid || h.school_year?.uuid
        const glUuid = h.gradeLevel?.uuid || h.grade_level?.uuid
        return syUuid === blk.meta?.schoolYearUuid && glUuid === blk.meta?.gradeLevelUuid
    })
    
    if (!matchingHistory) return false
    
    // Find course in history matching by academic_course_uuid or course title
    const courses = matchingHistory.courses || []
    const matchingCourse = courses.find(c => {
        const courseUuid = c.academicCourseUuid || c.academic_course_uuid
        const courseTitle = c.courseTitle || c.course_title || ''
        // Match by academic course UUID (preferred) or by course title
        return (course.academicCourseUuid && courseUuid === course.academicCourseUuid) ||
               (courseTitle && courseTitle === course.title)
    })
    
    if (!matchingCourse) return false
    
    // Get mark_as_complete from meta
    const meta = matchingCourse.meta || {}
    return meta.mark_as_complete ?? meta.markAsComplete ?? false
}
