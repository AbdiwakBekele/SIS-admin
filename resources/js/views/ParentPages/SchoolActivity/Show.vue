<template>
    <PageHeader v-if="activity.uuid" :title="$trans(route.meta.trans, { attribute: $trans(route.meta.label) })" :navs="[
        { label: activity.title, path: { name: 'ParentSchoolActivityShow', params: { uuid: activity.uuid } } },
    ]">
        <PageHeaderAction :url="`school-activity/${activity.uuid}/`" name="ParentSchoolActivity"
            ...
            <template #dropdown>
                <DropdownItem icon="fas fa-pencil" v-if="perform('schoolActivity:manage')"
                    @click="router.push({ name: 'ParentSchoolActivityEdit', params: { uuid: activity.uuid } })">
    ...
    <ParentTransition appear :visibility="true">
        <ShowItem :init-url="initUrl" :uuid="route.params.uuid" @setItem="setItem"
            @redirectTo="router.push({ name: 'ParentSchoolActivityList' })" :refresh="refreshItem"
