<template>
  <PageHeader :title="$trans('academics.class_enrollment.class_enrollment')" :navs="[
    { label: $trans('academics.class_enrollment.class_enrollment'), path: 'StudentClassEnrollmentWorkspace' },
  ]">
  </PageHeader>
...
// Define tabs relevant for student-side class enrollment
// Order: Course Catalog, Graduation Audit & Class Enrollment, Course Requests (can send)
const tabs = computed(() => {
  return [
    { label: 'Course Catalog', route: 'StudentCourseCatalog', icon: 'fa-solid fa-book' },
    { label: 'Graduation Audit & Class Enrollment', route: 'StudentGraduationAudit', icon: 'fa-solid fa-graduation-cap' },
    { label: 'Course Requests', route: 'StudentCourseRequest', icon: 'fa-solid fa-clipboard-list' },
  ]
})
</script>
