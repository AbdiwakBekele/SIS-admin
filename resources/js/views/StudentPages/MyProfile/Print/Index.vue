    ...
    <div class="mx-auto bg-white rounded-2xl border shadow-sm print:shadow-none print:border-0 m-2">
        ...
        <div class="p-4">
            ...
            <div v-else-if="selectedStudent && allAcademicRecords.length > 0" class="relative p-8 print:p-4 transcript-content" style="min-height: 11in;">
                ...
                <!-- Official School Seal, Accept and sign, Notes (Side by Side) -->
                <div class="flex gap-0">
                    <!-- Official School Seal (empty for unofficial) -->
                    <div class="border-2 border-blue-600 bg-white p-1.5 w-64 flex-shrink-0">
                        <div class="text-center">
                            <h3 class="font-bold mb-1 text-xs text-center bg-blue-600 text-white py-1">Official School Seal</h3>
                            <div class="border-2 border-blue-600 min-h-[100px] w-32 mx-auto flex items-center justify-center">
                                <span class="text-[10px] text-gray-400">Seal/Stamp</span>
                            </div>
                        </div>
                    </div>

                    <!-- Accept and sign -->
                    <div class="border-2 border-l-0 border-blue-600 bg-white p-1.5 flex-1">
...
// Load transcript data (student portal: current user from myProfile — no uuid in route)
async function loadTranscript() {
    loading.value = true
    try {
        // Fetch current student from my profile
        const myProfileResponse = await store.dispatch("student/studentProfile/myProfile")
        if (!myProfileResponse?.student) {
            loading.value = false
            return
        }
        selectedStudent.value = myProfileResponse.student
        const uuid = selectedStudent.value.uuid
        if (!uuid) {
            loading.value = false
            return
        }
...
// Actions
function goBack() {
    router.push({ name: 'MyStudentProfile' })
}
