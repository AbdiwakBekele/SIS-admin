<template>
    <ConfigPage>
        <template #action>
            <div class="space-x-4">
                <BaseButton
                    @click="router.push({ name: 'ConfigPermission' })"
                    >{{
                        $trans("config.permission.role_permission")
                    }}</BaseButton
                >
            </div>
        </template>
        ...
        >
            <CardHeader
                first
                :title="$trans('config.permission.user_permission_config')"
                :description="$trans('config.permission.user_permission_info')"
            ></CardHeader>
...
const search = async (query) => {
    return await store
        .dispatch("config/permission/search", {
            query,
        })
        .then((response) => {
            return response
...
const searchUser = async (query) => {
    return await store
        .dispatch("config/permission/searchUser", {
            query,
        })
        .then((response) => {
            return response
