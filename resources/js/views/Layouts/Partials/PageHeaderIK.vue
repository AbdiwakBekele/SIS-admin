        ...
        <div class="px-4 sm:px-6 lg:px-0">
            <div v-if="!noBreadcrumb">
                <nav class="sm:hidden" aria-label="Back">
                    <a href="#" @click="router.go(-1)"
                        ...
                        <div>
                            <i class="fas fa-chevron-left -ml-1 mr-1 h-5 w-5 shrink-0 text-gray-500"
                                aria-hidden="true"></i>
                            {{ $trans("general.back") }}
                        </div>
                    </a>
                    ...
                    <ol role="list" class="flex items-center space-x-4">
                        ...
                        <li v-for="nav in navs">
                            <div class="flex items-center">
                                <span class="w-5 shrink-0 text-gray-500 mx-0">/</span>
                                <router-link :to="generatePath(nav)"
                                    class="text-sm font-medium text-gray-400 hover:text-gray-600 mx-0">{{ nav.label }}</router-link>
                            </div>
                        </li>
                        <li>
                            <div class="flex items-center">
                                ...
                                <span aria-current="page"
                                    class="text-sm font-medium text-gray-400 mx-0">{{ title }}</span>
                            </div>
                        </li>
                    </ol>
                </nav>
            ...
            <div class="mt-2 md:flex md:items-center md:justify-between">
                <div class="min-w-0 flex-1 dark:text-gray-400">
                    <h2 class="text-xl font-bold leading-7 sm:truncate sm:text-2xl">
                        {{ title }}
                    </h2>
