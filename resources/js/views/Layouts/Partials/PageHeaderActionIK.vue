<template>
    <div class="flex space-x-2">
        ...
        <!-- List -->
        <BaseButton design="white" v-if="hasListAction" @click="router.push({ name: ListAction })">
        ...
        <BaseButton v-if="hasNewAction" design="white" @click="router.push({ name: NewAction })">
        ...
        <DropdownMenu no-width top-margin v-if="showBulkAction && bulkActions.length">
            ...
            <DropdownItem :icon="bulkAction.icon" v-for="bulkAction in bulkActions" :key="bulkAction.name"
                @click="$emit('onBulkAction', bulkAction.name)">{{ bulkAction.label }}</DropdownItem>
        </DropdownMenu>


        <!-- Print -->
        <BaseButton v-if="hasPrintAction" design="white" @click="generateOutput('print')">
            <i class="fa-solid fa-print"></i>
        </BaseButton>

        <!-- Refresh -->
        <!-- <BaseButton design="white" @click="emitter.emit('listItems')">
        ...
        <BaseButton design="white" v-tooltip="$trans('global._view', { attribute: $trans('general.list') })
            " v-if="hasViewAction && route.query.view != 'list'" @click="updateURL({ view: 'list' })">
            <i class="fas fa-list"></i>
        </BaseButton>

        <BaseButton design="white" v-tooltip="$trans('global._view', { attribute: $trans('general.card') })
            " v-if="hasViewAction && route.query.view == 'list'" @click="updateURL({ view: 'card' })">
        ...
        <DropdownMenu double-top-margin :right-margin="false" no-width
            v-if="route.query.view != 'board' && columnsWithLabel.length" v-tooltip="$trans('global.select', { attribute: $trans('general.column') })
                ">
            <template #clickable>
                <BaseButton design="white">
            ...
            <div v-for="(column, index) in columnsWithLabel" :key="column.key" class="flex px-4 py-2 text-sm">
                <BaseCheckbox v-model="column.visibility" :disabled="index === 0 ? true : false" :label="column.label"
                    @update:modelValue="
                        onColumnVisibilityChange(columnsWithLabel)
                        " />
            </div>
        </DropdownMenu>
        ...
        <DropdownMenu double-top-margin :right-margin="false" no-width v-if="dropdownActions.length || $slots.dropdown">
            <template #clickable>
                <BaseButton design="white">
                    ...
                    <i class="fa-solid fa-download py-1"></i>
                </BaseButton>
            </template>
            <!-- <DropdownItem icon="fas fa-download" v-if="hasImportAction" @click="$emit('toggleImport')">
                {{ $trans("general.import") }}
            </DropdownItem> -->
            <!-- <DropdownItem icon="fas fa-sync" @click="emitter.emit('listItems')">{{ $trans("general.refresh") }}
            </DropdownItem> -->
            <!-- <DropdownItem icon="fas fa-print" v-if="hasPrintAction" @click="generateOutput('print')">
                {{ $trans("general.print") }}
            </DropdownItem> -->
