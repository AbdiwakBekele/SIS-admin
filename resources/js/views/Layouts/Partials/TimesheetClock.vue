<template>
    ...
    >
        ...
        <button
            ...
            @click="clock"
            class="relative inline-flex items-center border px-2 py-1 sm:px-4 sm:py-2 text-sm font-medium"
            :class="{
            ...
            <div class="space-x-2">
                ...
                <span v-if="!screenSize.small"
                    >{{
                        state.timesheet?.clockIn
                            ? $trans(
                                  "employee.attendance.timesheet.props.clock_in"
                              )
                            : $trans(
                                  "employee.attendance.timesheet.props.clock_out"
                              )
                    }}
                </span>
                <span v-else>
...
const allowEmployeeClockInOut = computed(
    () => getConfig("employee.allowEmployeeClockInOut").value
)

const enableGeolocationTimesheet = computed(
    () => getConfig("employee.enableGeolocationTimesheet").value
)
...
const checkTimesheet = async () => {
    ...
    await store
        .dispatch("employee/attendance/timesheet/check")
        .then((response) => {
...
const clock = async () => {
    ...
    // console.log(getConfig("employee.cacheGeolocationData").value)
    ...
    if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(
            ...
            handleLocationError,
            { maximumAge: getConfig("employee.cacheGeolocationData").value }
        )
    } else {
        showError(
            $trans("employee.attendance.timesheet.geolocation_not_supported")
        )
        isProcessing.value = false
    }
}
...
const storeTimesheet = async (location = {}) => {
    await store
        .dispatch("employee/attendance/timesheet/clock", {
...
const handleLocationError = (error) => {
    showError(
        $trans("employee.attendance.timesheet.unable_to_detect_geolocation")
    )
    isProcessing.value = false
}
