...
import { flattenModules } from "@core/helpers/moduleList"

function filterAccessibleNavigations(routes, moduleList) {
    const accessibleNavigations = routes.filter((route) => {
        let isAccessible = false

        const moduleExists = moduleList.find(
            (m) =>
                m.name === route.name ||
                m.name + "List" === route.name ||
                m.name + "Index" === route.name
        )
        if (moduleExists && !moduleExists.visibility) {
            return false
        }
        ...
        if (isAccessible) {
            if (route.children && route.children.length) {
                route.children = filterAccessibleNavigations(
                    route.children,
                    moduleList
                )
            }
...
const navigation = {
    ...
    actions: {
        setNavigation({ commit }) {
            const moduleList = flattenModules(
                store.getters["config/configs"].module
            )
            const navigations = filterAccessibleNavigations(
                _.cloneDeep(routes),
                moduleList
            )
