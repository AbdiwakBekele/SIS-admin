...
const user = {
    ...
    actions: {
        ...
        async impersonate({ commit, dispatch, getters }, payload) {
            await Api.custom({
                url: "app/users/" + payload.uuid + "/impersonate",
                method: "POST",
            })
                .then((response) => {
                    commit("RESET_STATE")
                    dispatch("setUser", response.user, { root: true })
                })
                .catch((error) => {
                    commit("SET_FORM_ERRORS", Form.getErrors(error))
                    throw error
        ...
        async unimpersonate({ commit, dispatch, getters }, payload) {
            await Api.custom({
                url: "app/users/unimpersonate",
                method: "POST",
            })
                .then((response) => {
                    commit("RESET_STATE")
