<template>
    ...
    <div class="mt-1 lg:flex lg:space-x-2">
        <div
            ...
            class="border-primary mx-1 my-2 flex-1 cursor-pointer rounded-lg border-2 dark:border-site-primary"
            :class="{
                'text-xs px-2 py-1': size == 'xs',
                'text-sm px-3 py-2': size == 'sm',
                'text-base px-4 py-3': size == 'md',
                'bg-primary text-white dark:bg-site-primary': isSelected(
                    option.value
                ),
        ...
        >
            <div class="flex h-full items-center justify-between align-middle">
                ...
                <span v-if="isSelected(option.value)" class="ml-1"
