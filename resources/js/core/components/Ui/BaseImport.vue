<template>
    <BaseCard :is-loading="isLoading">
        ...
        <template #action>
            <BaseButton
                @click="showHistory"
                size="xs"
                v-if="historyPath && !state.historyLoaded"
            >
    ...
    <BaseCard
        ...
        v-if="state.historyLoaded"
    >
        <template #title>
            {{ $trans("general.history") }}
        </template>

        <div v-if="state.historyLoaded && state.history.length">
            <SimpleTable :header="historyHeader">
                <DataRow v-for="item in state.history" :key="item.uuid">
                    <DataCell name="count">
                        {{ item.count }}
                    </DataCell>
                    <DataCell name="importedAt">
                        {{ item.importedAt.formatted }}
                    </DataCell>
        ...
        <div
            class="px-4 py-2"
            v-else-if="state.historyLoaded && !state.history.length"
        >
...
import { actingAs } from "@core/helpers/action"
...
const historyHeader = [
    ...
    {
        key: "importedAt",
        label: $trans("general.imported_at"),
        visibility: true,
    },
    {
        key: "action",
...
const showHistory = async () => {
    ...
    store
        .dispatch("moduleImport/history", { path: props.historyPath })
        .then((response) => {
...
const deleteHistory = async (uuid) => {
    ...
    isLoading.value = true
    store
        .dispatch("moduleImport/deleteHistory", {
