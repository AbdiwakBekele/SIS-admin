...
function getCurrency(currencyName = "") {
    ...
    if (!currencyName) {
        currencyName = store.getters["config/config"]("system.currency")?.name
    }
...
export function formatAmount(amount = 0, currencyName = "") {
    let currency = getCurrency(currencyName)

    return roundNumber(amount, currency.decimal || 2)
}

export function formatCurrency(amount = 0, currencyName = "") {
    let currency = getCurrency(currencyName)
